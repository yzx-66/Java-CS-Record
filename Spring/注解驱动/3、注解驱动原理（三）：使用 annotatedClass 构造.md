...
## ConfigurationClassPostProcessor 加载 bean
* 最后层层调用下去，还是用的 ClassBeanDefiontionScanner.scan
...
...
    do {
			...
			// 解析剩下的注解（@ConpomentScan），然后加载 beanDefition
...
会走到这：ConfigurationClassParser.doProcessConfigurationClass(ConfigurationClass, SourceClass)
```java
